...
class VisualAnalyzerGUI(tk.Tk):
    ...
    def populate_analysis_tab(self):
        ...
        if self.debug_mode:
            ...
            tk.Checkbutton(
                bool_frame, text="Apply Mask", variable=self.apply_mask_var
            ).pack(anchor=tk.W)
            mask_sub_frame = ttk.Frame(bool_frame)
            mask_sub_frame.pack(anchor=tk.W, fill=tk.X, padx=(20, 0))
            tk.Checkbutton(
                mask_sub_frame,
                text="Mask BG is White",
                variable=self.mask_bg_is_white_var,
            ).pack(anchor=tk.W)

            tk.Checkbutton(
                bool_frame, text="Symmetry Analysis", variable=self.symmetry_var
            ).pack(anchor=tk.W)
            ...
            tk.Entry(param_frame, textvariable=self.masking_order_var).pack(
            ...
            tk.Checkbutton(blur_frame, text="Blur", variable=self.blur_var).pack(
                anchor=tk.W
            )
            blur_options_frame = ttk.Frame(blur_frame)
            blur_options_frame.pack(fill=tk.X, padx=(20, 0))
            tk.Label(blur_options_frame, text="Blur Kernel (W H, e.g., 5 5):").pack(
                anchor=tk.W
            )
            tk.Entry(blur_options_frame, textvariable=self.blur_kernel_var).pack(
                anchor=tk.W, fill=tk.X
            )
            ...
            tk.Checkbutton(
                agg_frame, text="Aggregate", variable=self.aggregate_var
            ).pack(anchor=tk.W)
            ...
            tk.Entry(agg_options_frame, textvariable=self.agg_kernel_size_var).pack(
                anchor=tk.W, fill=tk.X
            )
            tk.Label(agg_options_frame, text="Agg. Min Area:").pack(anchor=tk.W)
            tk.Entry(agg_options_frame, textvariable=self.agg_min_area_var).pack(
                anchor=tk.W, fill=tk.X
            )
            tk.Label(agg_options_frame, text="Agg. Density Thresh:").pack(anchor=tk.W)
            tk.Entry(agg_options_frame, textvariable=self.agg_density_thresh_var).pack(
                anchor=tk.W, fill=tk.X
            )

        tk.Button(
            main_frame, text="Run Analysis", command=self.run_analysis_dialog
    ...
    def populate_create_project_tab(self):
        ...
        tk.Label(create_frame, text="New Project Name:").pack(
            side=tk.LEFT, padx=5, pady=5
        )
        tk.Entry(create_frame, textvariable=self.new_project_name_var, width=50).pack(
            side=tk.LEFT, expand=True, fill=tk.X, padx=5, pady=5
        )
        tk.Button(create_frame, text="Create", command=self.handle_create_project).pack(
            side=tk.LEFT, padx=5, pady=5
        )
    ...
    def populate_manage_dataset_tab(self):
        ...
        tk.Label(manage_frame, text="Select Project:").pack(
            side=tk.LEFT, padx=5, pady=5
        )
        self.manage_project_combobox = ttk.Combobox(
            manage_frame,
            textvariable=self.manage_project_var,
            values=self.available_projects,
        )
        self.manage_project_combobox.pack(
            side=tk.LEFT, expand=True, fill=tk.X, padx=5, pady=5
        )
        if self.available_projects:
    ...
    def launch_dataset_manager(self):
        ...
        try:
            project_files = self.project_manager.get_project_file_paths(
                project_name, debug_mode=self.debug_mode
            )
            ...
            if not dataset_image_paths:
                messagebox.showinfo(
                    "Info",
                    rf"No training images found in project '{project_name}'. Please add images to the 'training' folder and ensure the 'training_path' in project_config.json is correct.",
                )
                return
            ...
            DatasetManagerGUI(
                dataset_manager_window, dataset_image_paths, str(config_file_path)
            )
            self.wait_window(dataset_manager_window)

            messagebox.showinfo(
                "Dataset Manager",
                "Dataset management complete. Remember to re-run analysis to use updated color space.",
            )

        except Exception as e:
            messagebox.showerror(
                "Error",
                f"An error occurred while trying to open the dataset manager: {e}",
            )
    ...
    def run_analysis_dialog(self):
        if self.debug_mode:
            debug_choice = messagebox.askyesno(
                "Run Mode", "Do you want to run the pipeline in Debug Mode?"
            )
    ...
    def run_analysis(self):
        ...
        if args.color_alignment and not args.sample_color_checker:
            messagebox.showerror(
                "Error", "Please select a color checker image for Color Alignment."
            )
            return
        ...
        args.skip_report_generation = False
        try:
            args.agg_kernel_size = (
                int(self.agg_kernel_size_var.get())
                if self.agg_kernel_size_var.get()
                else 7
            )
            args.agg_min_area = (
                float(self.agg_min_area_var.get())
                if self.agg_min_area_var.get()
                else 0.0005
            )
            args.agg_density_thresh = (
                float(self.agg_density_thresh_var.get())
                if self.agg_density_thresh_var.get()
                else 0.5
            )
        ...
        try:
            messagebox.showinfo(
                "Running Analysis", "Analysis is starting. This may take a moment."
            )
            run_analysis(args)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode for GUI layout."
    )
