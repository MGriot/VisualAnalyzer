            ...
            if os.path.isdir(args.image):
                for filename in os.listdir(args.image):
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        process_image(image_path, args, project_manager, color_corrector, color_analyzer, report_generator, lower_hsv, upper_hsv, center_hsv, correction_matrix, dataset_debug_info)
            else:
                process_image(args.image, args, project_manager, color_corrector, color_analyzer, report_generator, lower_hsv, upper_hsv, center_hsv, correction_matrix, dataset_debug_info)
            ...
            if os.path.isdir(args.video):
                for filename in os.listdir(args.video):
                    if filename.lower().endswith(('.mp4', '.avi', '.mov')):
                        ...
                        process_video(video_path, args, color_corrector, color_analyzer, report_generator, lower_hsv, upper_hsv, center_hsv, correction_matrix, dataset_debug_info)
            else:
                process_video(args.video, args, color_corrector, color_analyzer, report_generator, lower_hsv, upper_hsv, center_hsv, correction_matrix, dataset_debug_info)
...
def process_image(image_path, args, project_manager, color_corrector, color_analyzer, report_generator, lower_hsv, upper_hsv, center_hsv, correction_matrix, dataset_debug_info):
    ...
    try:
        ...
        if args.blur:
            ...
            if args.debug:
                path = str(report_generator.project_output_dir / "blurred_debug.png")
                save_image(path, image_for_processing)
                debug_image_pipeline.append({"title": f"{pipeline_step_counter}. After Blurring", "path": os.path.basename(path)})
                pipeline_step_counter += 1
            ...
            if args.symmetry:
                ...
                for key, value in symmetry_results.items():
                    ...
                    if 'chunks' in value or 'reconstruction' in value:
                        ...
                        plot_path = report_generator.project_output_dir / plot_filename
                        plt.savefig(plot_path)
                        plt.close(fig)
                        if os.path.exists(plot_path):
                            print(f"[DEBUG] Saved symmetry plot: {plot_path}")
                            debug_data_for_report["symmetry_visualizations"].append({"title": f"{key.replace('_', ' ').title()} (Score: {value['score']:.4f})", "path": plot_filename})
